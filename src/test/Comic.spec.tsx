...
describe('메인페이지 테스트', () => {
  test('스크롤이 끝까지 내려가면 다음페이지를 렌더링한다.', async () => {
    ...
    await simulateScrollEvent(4);
    const newContentElement = screen.getByText(/데스크레센도/i);
    expect(newContentElement).toBeInTheDocument();
  });

  test('페이지가 처음 로딩되면 1페이지를 렌더링한다', async () => {
    await act(async () => render(<ComicsContainer genre="drama" />, { wrapper }));
    const newContentElement = screen.getByText(/시크릿 동맹/i);
    expect(newContentElement).toBeInTheDocument();
  });
  ...
  test('연재중 필터가 클릭되면 완결 웹툰은 렌더링 되지 않는다', async () => {
    ...
    const completedElements = screen.getAllByText(/완결/i).filter(i => i.tagName !== 'BUTTON');
  ...
  test('완결 필터가 클릭되면 연재중 웹툰은 렌더링 되지 않는다', async () => {
    ...
    fireEvent.click(button!);
    const completedElements = screen.getAllByText(/연재/i).filter(i => i.tagName !== 'BUTTON');
    expect(completedElements.length).toBe(0);
  });
